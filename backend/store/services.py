...
def create_order(*, user: User, items: list[dict], shipping: dict, payment_method: str, idempotency_key: str) -> Order:
    ...
    with transaction.atomic():
        ...
        for product, variant, color, quantity, line_total in resolved:
            OrderItem.objects.create(order=order, product=product, name=product.name, color=color, image=(variant.images[0] if variant and variant.images else (product.images[0] if product.images else "")), quantity=quantity, price=product.price, subtotal=line_total)
